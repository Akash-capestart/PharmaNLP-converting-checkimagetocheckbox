...
import { SplitedDropDown } from "../../../commonComponents/SplitedDropDown";
...
export function AdvanceSearchComponent({
  ...
  const enteredTermsHandler = (e: string) => setadvanceSearchState({
    ...advanceSearchState,
    enteredSearchTerm: e,
    termsQueryBoxPopOverShow: false,
  });
  ...
  const enteredQueryHandler = (e: string) => setadvanceSearchState({
    ...advanceSearchState,
    querySet: e,
    termsQueryBoxPopOverShow: false,
    searchQueryBoxPopOverShow: false,
  ...
  const queryBoxDropDownValueHandler = (val: string) => setadvanceSearchState({
    ...advanceSearchState,
  ...
  const concatinateQueryHandler = (val: string) => {
    ...
    copyOfAdvanceSearchFieldState["termsHaveSelected"].forEach((each, idx) => {
      const termHasSelectedJoint = `${"("}${copyOfAdvanceSearchFieldState["enteredQueryTerms"][idx]
        }${"["}${each}${"]"}${")"}`;
      const newEntry = [
        ...dropDownSelectedItemsQueryArray,
        termHasSelectedJoint,
      ];
      const singleEntry = `${copyOfAdvanceSearchFieldState["enteredQueryTerms"][idx]
        }${"["}${each}${"]"}`;
    ...
    if (copyOfAdvanceSearchFieldState["andOrNotQuery"].length === 0) {
      ...
      copyOfAdvanceSearchFieldState["andOrNotQuery"].forEach((each, idx) => {
        if (searchQueryArray.length === 0) {
          const queryString = `${dropDownSelectedItemsQueryArray[idx]
            } ${each} ${dropDownSelectedItemsQueryArray[idx + 1]}`;
          const newQueryEntry = [queryString];
          searchQueryArray = newQueryEntry;
        } else {
          const queryString = `${"("}${searchQueryArray[0]}${")"} ${each} ${dropDownSelectedItemsQueryArray[idx + 1]
            }`;
          const newQueryEntry = [queryString];
          searchQueryArray = newQueryEntry;
        }
  ...
  const crossClickHandler = (key: string) => {
    if (key === "enteredSearchTerm") setadvanceSearchState({
      ...advanceSearchState,
      [key]: "",
    });
    else setadvanceSearchState({
                ...
                {!advanceSearchState.enteredSearchTerm &&
                  advanceSearchState.termsQueryBoxPopOverShow && (
                    <div className="position-absolute popover-position">
                      <PopOver text={"Please Enter the term..."} count={null} width={"auto"} />
                    </div>
                  )}
                {advanceSearchState.enteredSearchTerm && (
                  <div
