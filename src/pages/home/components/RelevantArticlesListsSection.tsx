...
import {
  fetchAllRelevantArticles,
  fetchRelevantArticlesByKeyWords,
} from "../../../redux/actions/RelevantArticlesActions";
import { useAppSelector, useAppDispatch } from "../../../redux/Hooks";
import { addRelevantActivePageNumber } from "../../../redux/reducers/RelevantArticlesSlice";
...
export function RelevantArticlesListsSection({
  ...
  const { midFont, lowFont } = useAppSelector(
    (state) => state.globalFontResizer
  );
  ...
  const enterKeyPressHandler = (
    userPressedKey: React.KeyboardEvent<HTMLElement>,
    refId: number
  ) => {
  ...
  const { activePage, searchKeyWord } = useAppSelector(
    (state) => state.relevantArticlesDetails
  );
  const totalArticlesCount = 40;
  ...
  const pageChangeArticlesFetchHandler = async (pageNo: number) => {
    if (pageNo === activePage) scrollToTopHandler(110, "smooth");
      ...
      if (searchKeyWord)
        dispatch(
          fetchRelevantArticlesByKeyWords({
            endUrl: `/article/getArticlesByVal?value=${searchKeyWord}&page=${pageNo}`,
          })
        );
      else
        dispatch(
          fetchAllRelevantArticles({
            endUrl: `/article/getAllArticles?page=${pageNo}`,
          })
        );
    }
  };

  const handleChange = (event: any, refid: any) => {
    if (event.target.checked) {
      console.log("✅ Checkbox is checked");
      articleSelectHandler(refid);
    } else {
  ...
  return (
    <>
      {newObj &&
        newObj.map((article: any, idx: number) => {
          return (
            <div
              key={idx}
            ...
            >
              <div className="col-md-10 no-padding">
                <div className="d-flex align-items-center">
                  <div className="cursor-pointer">
                    <label>
                      <input
                        type="checkbox"
                        checked={selectedArticles.includes(article.refId)}
                        onChange={(event) => {
                          handleChange(event, article.refId);
                        }}
                      />
                      <span></span>
                    </label>
                ...
                <div
                  ...
                  style={{
                    maxHeight: articlesExpand ? 300 : 60,
                  }}
                >
                  <p
                  ...
                  {activeIdToAddTag === article.refId ? (
                    <>
                      <div className="position-relative pad-r-10 btn-width-animation">
                        <input
                          placeholder="Add Tag"
                          className="tag-add-input-active w-100"
                          autoFocus
                          onChange={(e) =>
                            tagInputChangeHandler(e.target.value)
                          }
                          onKeyDown={(e) =>
                            enterKeyPressHandler(e, article.refId)
                          }
                        />
                        {activeIdToAddTag === article.refId && (
                          <img
                            ...
                            alt="Cancel..."
                            className="w-15 position-absolute add-tag-close-icon cursor-pointer"
                            onClick={() => tagInputHideHandler()}
                          />
                        )}
                      </div>
                      <img
