...
const MultiSelectView = ({ label, itemLists, filterHadler, checkValue }: MultiSelectViewProps) => {
  ...
  const filterContentClickHandler = (key: string, val: string) => filterHadler(key, val);
...
export function FilteringComponent({
  ...
  const filterResetHandler = () => setmultiSelectState({
    ...multiSelectState,
    Source: ["All"],
    Country: ["All"],
    Language: ["All"],
  ...
  const filterContentClickHandler = (key: string, val: string) => {
    if (!multiSelectState[key].includes(val)) {
      if (val === "All") setmultiSelectState({
        ...multiSelectState,
        [key]: [val],
      });
      else {
  ...
  const filterSetHandler = () => advanceSearchAndFilterShowHandler();
