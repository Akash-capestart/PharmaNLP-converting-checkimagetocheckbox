...
export function FontSlider() {
  ...
  const fontChangeShowHandler = () => setfontSliderState((prevVal) => ({
      ...prevVal,
  ...
  const activeFontSelectingHandler = (e:string) => {    
    ...
    let findIndexOfPrev = fontVal.findIndex((each) => each === fontSliderState["fontSelectedVal"]);
    setfontSliderState({
      ...fontSliderState,
      fontSelectedVal: parseInt(e),
      fontLabel: fontArr[findIndexOfE],
    });
    if (parseInt(e) < fontSliderState["fontSelectedVal"]) dispatch(fontSizeDecrement({ fontValue: findIndexOfPrev - findIndexOfE }));
    else dispatch(fontSizeIncrement({ fontValue: findIndexOfE - findIndexOfPrev }));
  };
